#   Build Analyzer v1.0 (CH32V003A4M6)
#   Автор: Волков Олег
#   Дата создания скрипта: 18.03.2025
...
$sections = @('.init', '.highcodelalign', '.highcode', '.text', '.fini', '.dalign', '.dlalign', '.data', '.bss', '.stack')
...
# Расчет FLASH и RAM
[int]$ramUsed = $sectionSizes['.highcode'] + $sectionSizes['.dalign'] + $sectionSizes['.data'] + $sectionSizes['.bss']+ $sectionSizes['.stack']
[int]$flashUsed = $sectionSizes['.init'] + $sectionSizes['.highcodelalign'] + $sectionSizes['.highcode'] + $sectionSizes['.text'] + $sectionSizes['.fini'] + $sectionSizes['.dalign'] + $sectionSizes['.dlalign']+ $sectionSizes['.data']+ $sectionSizes['.bss']
